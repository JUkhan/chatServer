...
const ChatWindow: React.FC<Params> = ({
  ...
  const io = useSignalrConnectionHook(
    hubUrl,
    (listenningOn: string, data: any) => {
      switch (listenningOn) {
        case "connected":
        ...
        case "activeUser":
          setState((pre) => ({
            ...pre,
            groups: data.groups,
            unreadStatus: data.unreadStatus,
          }));
        ...
        case "groupMessage":
          setState((pre) => {
            const len = pre.msgObj[data.groupName]
              ? pre.msgObj[data.groupName].length
              : 0;
            ...
            if (gname.endsWith("All Users")) {
              if (
                !pre.groups[0].groupName.includes(
                  gname.replace("All Users", "")
                )
              ) {
                return pre;
              }
            ...
            if (pre.selectedGroup.groupName !== data.groupName) {
              ...
              if (typeof popupMessage === "function") {
                popupMessage({
                  title: `${getGroupName(data)} - ${data.userName}`,
        ...
        case "newGroup":
          setState((pre) => {
            ...
            if (!pre.groups[0].groupName.includes(match)) {
              return pre;
            }
            if (!data.id) {
              if (typeof popupMessage === "function") {
                popupMessage({
                  title: getGroupName(data),
                  message: "Already exist",
                });
              } else {
                toast.info(`"${getGroupName(data)}" already exist`);
              }
              return pre;
            }
  ...
  useEffect(() => {
    ...
    setHeight(() => [
      inputRef.current?.clientWidth ?? 700,
      window.innerHeight - bufferHeight,
    ]);
    const handleSize = () => {
      setHeight(() => [
        inputRef.current?.clientWidth ?? 700,
  ...
  const sendMessage = () => {
    io.current?.invoke("GroupMessage", currentUser, {
      ...selectedGroup,
      message,
    } as Group);
    setMessage("");
  ...
  return (
    <>
      ...
      <TooltipProvider>
        <div
          style={{ height: hideWindow ? 0 : height }}
              ...
              >
                <div className="flex flex-col items-center">
                  <div style={{ width: width + 60 }}>
                    {msgObj[selectedGroup.groupName]?.map(
                      (m: any, i: number) => (
                        <div key={m.id}>
                          <div
                            className={cn({
                          ...
                          >
                            {i > 0 &&
                            m.userName ==
                              msgObj[selectedGroup.groupName][i - 1].userName &&
                            (getTime(new Date(m.createdAt)) -
                              getTime(
                                new Date(
                                  msgObj[selectedGroup.groupName][
                                    i - 1
                                  ].createdAt
                                )
                              )) /
              ...
              <div className="flex items-center justify-center">
                <Input
                  ...
                  onKeyUp={(ev) => {
                    if (
                      ev.key === "Enter" &&
                      !!(selectedGroup?.id && message && currentUser)
                    ) {
                      sendMessage();
                    }
                  }}
